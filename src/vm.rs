...
pub fn run(&mut self, code: &[u8], trace: bool) -> Result<(), String> {
        self.ip = 0;
        ...
        loop {
            ...
            if trace {
                println!("ip=0x{:04X}  {:?}  stack={:?}", self.ip, op, self.stack);
            }
            ...
            match op {
                ...
                Op::Div => {
                    ...
                    let a = self.pop()?;
                    if b == 0 {
                        return Err(format!("trap at ip=0x{:04X}: division by zero", self.ip));
                    }
                    self.push(a / b)?;
                }
                Op::Mod => {
                    ...
                    let a = self.pop()?;
                    if b == 0 {
                        return Err(format!("trap at ip=0x{:04X}: mod by zero", self.ip));
                    }
                    self.push(a % b)?;
                }
                Op::Neg => {
